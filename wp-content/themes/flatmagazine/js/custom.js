...
"use strict";
      ...
      jQuery(".pb_1_1").each(function() { 
        ...
        var themecolor = jQuery('body').attr('data-themecolor');
  ...
  jQuery('.piechart').easyPieChart({
		...
		var chart = window.chart = jQuery('.piechart').data('easyPieChart');
      ...
      jQuery("#main-nav > div > ul > li > a").each(function() {
        ...
        fcont = fcont.replace(/<\/?[^>]+>/g, '');
        jQuery(this).attr('data-hover',fcont).addClass('firstlevel');
      });
      ...
      jQuery(".span3 a").each(function(){
        var flink = jQuery(this).attr('href','#');
      });
      
      
	jQuery('.close-nav, .close-sidebar, .sidebar-close').click(function(){
		snapper.close();
		return false;
	});

	jQuery('.dropdown-nav').click(function(){
		jQuery(this).parent().find('.nav-item-submenu').toggle(200);
		jQuery(this).parent().find('.dropdown-nav').toggleClass('dropdown-nav-inactive');
	});
	
	jQuery('.wide-image').click(function(){
		jQuery(this).parent().find('.wide-item-content').toggle(50);
		return false;
	});

    if (!jQuery('body').hasClass('nomobilebar')) {		
    
	var snapper = new Snap({
	  element: document.getElementById('container')
	});
    snapper.on('enable', function(){});

	jQuery('#showrightmenu').click(function(){
    ...
    function pflfix() {
        ...
        pfl.each(function() {  var h = jQuery(this).height();  max = h > max ? h : max; });
        pfl.height(max);        
    }   
    pflfix();
    ...
    if (jQuery("body").hasClass('boxcontainer')) {
        retinaready();
    }
   ...
   jQuery(window).resize( function(){
    ...
    jQuery('.smartquee').liScroll({travelocity: 0.05});
    ...
    jQuery(this).attr('href',(jQuery(this).children().attr('src').replace('-150x150', '')));
    jQuery(this).attr('data-rel','magnific');
    });
                ...
                if (jQuery("body").hasClass('fixedheader')) {
                        /*if (jQuery('#megaslider').length) {
                            if (jQuery(window).scrollTop()>0 ) { 
                                c.height((b)-jQuery(window).scrollTop()); 
                            } else { 
                                c.height(''); 
                            }
                        }*/
